 ...
 */
	...
	// $kd17    = $out_load.'\Kdno17.class.php';
	$kdfile  = $out_load.'\Kdno17.conf.php';
	// require_once($kd17);//加载类
	...
	/* Jie 20160921 */
	// 线程名称
	$node = isset($_GET['node']) ? $_GET['node'] : 'US_zhongtong';//默认 MG_zhongtong 美国中通
...
//============= 测试 End  ======== 
		...
		foreach($nu_list as $kk=>$item){
			...
			//调用文件中的函数处理
			$res[$kk] = getArr($tracking_number, $debug=0, $serurl, $config);//$debug 调试编码，不同数值可用于检查不同位置的数据输出
