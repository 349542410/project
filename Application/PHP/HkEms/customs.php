...
*/
	...
	$node = isset($_GET['node']) ? $_GET['node'] : 'Hk_ems_log';//默认 Hk_ems 香港E特快
	...
	//查tran_list.trankd=? IL_State > 999 的 MK单进行物流信息获取
	$find_sql = "SELECT $no,id FROM mk_tran_list WHERE trankd = '$TranKd' AND IL_state > 999 AND IL_state <> '1003' AND id > $maxlid AND $no <> '' ORDER BY id ASC LIMIT $limit";
// print_r($find_sql);
// die;
...
//============= 测试 End  ======== 
		...
		foreach($nu_list as $kk=>$item){
			...
			//调用文件中的函数处理
			$res[$kk] = getData($tracking_number, $EMS, $config, $pdo, $TranKd);
// print_r($res[$kk]);
// die;
			if($res[$kk]['do'] == 'yes'){
				$et++;
		...
		if($et == 0){
			...
			$backXML = '<Response service="RoutePushService"><Head>OK</Head>请求发送总数：'.$limit.'个，实际查询数据：'.count($nu_list).'个；成功保存海关状态信息：'.$et.'个；耗时：'.$Ttime.'秒；反馈信息：'.$msg.'</Response>';
		}
		echo $backXML;
