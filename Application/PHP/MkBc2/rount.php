 ...
 */
	...
	require_once('deal.php'); 		//获取xml并转数组，数组处理
	$bc2 = new \bc2();
	...
	/* Jie 20160921 */
	// 线程名称
	$node = isset($_GET['node']) ? $_GET['node'] : 'MkBc2';//默认 MkBc2 美快BC优选2
...
//============= 测试   ========
		...
		// $nu_list =  array('0'=>array('STNO'=>'9973508782447'));//新 测试用
...
//============= 测试 End  ======== 
		...
		foreach($nu_list as $kk=>$item){
			...
			//调用文件中的函数处理
			$res[$kk] = getArr($tracking_number, $serurl, $bc2);//$debug 调试编码，不同数值可用于检查不同位置的数据输出
