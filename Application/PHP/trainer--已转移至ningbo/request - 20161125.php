...
1.2 如果mk_trainer_notes已有数据，则取最新的数据，分析：若NextPage > 0，则NextPage+1，开始时间取其StartTime，结束时间取其EndTime；
 ...
 */
	...
	$maxlid_sql = "select StartTime,EndTime,NextPage from mk_trainer_notes order by id desc limit 1";
	...
	if($max->rowCount() > 0){
		...
		if($maxinfo['NextPage'] == '0'){
			echo '上一次查询反馈【没有下一页数据】，现在重新从第一页开始查询...';
			// exit;
			//考虑加入 当为0 的时候，是否要重新执行查询
			$StartTime = $time - 86400;// 一天前 减去一天的时间，以此作为开始时间
	...
	<Body>
		<Mft>
			<MftNo>1111111111</MftNo>
			<OrderNo>222222222222</OrderNo>
			<LogisticsNo>333333333333</LogisticsNo>
			<CheckFlg>0</CheckFlg>
	...
	if(isset($result['Message']['Header']['Result']) && $result['Message']['Header']['Result'] == 'T'){
		...
		if($check_arr > 1){
			...
			echo '操作完成！返回数据总数为：'.$count.'条；成功保存/更新：'.$sus.'条。';
