...
/*
	...
	修改日期：2016-11-28
	用途：自动去查询申报单的信息并保存
	新增数据表：mk_trainer_notes  mk_trainer_logs   mk_trainer
	涉及数据表：mk_il_logs  mk_logs  mk_transit_no   mk_transit_center  mk_tran_list
	指导文档：报关资料获取.xlsx
编写思路：
...
/*	// 测试数据
		...
		if($check_arr > 1){
			...
			foreach($data as $key=>$item){
				...
				if(isset($item['LogisticsNo']) && $item['LogisticsNo'] != ''){
					$save = save($item,$StartTime,$EndTime,$pdo,$erp_url,$erp,$TranKd);
			...
			if(isset($data['LogisticsNo']) && $data['LogisticsNo'] != ''){
				$save = save($data,$StartTime,$EndTime,$pdo,$erp_url,$erp,$TranKd);
			}else{
				$save = true;
			}
	...
	/*
		数据保存或更新
	 */
	function save($data,$StartTime,$EndTime,$pdo,$erp_url,$erp,$TranKd){
		...
		if($res_mkil->rowCount() > 0){
			...
			$set .= 'StartTime,EndTime,TranKd';
			$val .= "'".$StartTime."','".$EndTime."','".$TranKd."'";//记录当前查询的页数和结束时间   //新增TranKd 20170205 jie
			$set = rtrim($set, ',');//清除最右侧的英文逗号
...
/*
		20161213 jie 暂时注销不使用  标示一
		// 发送到ERP
		// 预校验标识不通过的，则发送到ERP，其它不用
		if($data['CheckFlg'] == '0'){
			$send = sendERP($data,$pdo,$erp_url,$erp);
		}*/

		// 只把状态为1002 发送到ERP  20161213 jie 暂时使用  标示一启用的时候，此处关闭
		$yy_arr = array('13','23','25','99');

		if(in_array($data['Status'],$yy_arr)){
			$send = sendERP($data,$pdo,$erp_url,$erp);// 20161213 jie  暂时使用符合在此数组内的判断方式进行发送erp   标示一启用的时候，此处关闭
		}

		$s_arr = array('00','02','24');  //把1400,1410  标示一启用的时候，此处关闭
		// $s_arr = array('00','02','13','23','25','99','24'); // 标示一

		if(in_array($data['Status'],$s_arr)){
			$step_two = save_step2($data, $pdo);// 20161208 Jie
		}
	...
	/**
	 * 给mk_tran_list，mk_logs，mk_il_logs保存数据
	 * @param  [type] $data [description]
	 * @param  [type] $pdo  [description]
	 * @return [type]       [description]
	 */
	function save_step2($data, $pdo){

		// 查询单号对应mk_tran_list的信息
		$check_tl = "SELECT `id`,`noid`,`CID`,`IL_state`,`MKNO`,`weight` FROM mk_tran_list WHERE `STNO` = '$data[LogisticsNo]'";

		$tran_list = $pdo->query($check_tl);

		if($tran_list->rowCount() > 0){
			$list = $tran_list->fetch(PDO::FETCH_ASSOC);

			// 修复 20170104 jie 当查询的状态已经为1003，即表示已签收，无需继续操作此单
			if($list['IL_state'] == '1003'){
				return '此单已被签收。';
			}

			// 查询单号对应 mk_transit_no 和 mk_transit_center 的信息
			$info_tl = "SELECT t.`no`,c.`transit` FROM mk_transit_no as t LEFT JOIN mk_transit_center as c ON t.`tcid`=c.`id` WHERE t.`id` = '$list[noid]'";

			$i_res = $pdo->query($info_tl);
			$info = $i_res->fetch(PDO::FETCH_ASSOC);

			//转换状态
			if($data['Status'] == '00' || $data['Status'] == '02'){
				$status = 1400;
				//$content = '【宁波口岸】发送订单信息到电子口岸'; //Man 170221
				$content = '发送订单信息到宁波电子口岸';
			}else if($data['Status'] == '24'){
				$status = 1410;
				$content = '【宁波口岸】海关货物放行';
			}else{
				$status = 1002;
				$content = '【宁波口岸】'.state_turn($data['Status']).'，详情请与我司或寄件人联系';
			}

			//如果申报单状态转化后=1002，则需要判断其预校验标识是否=0，若=0，则不执行后续的操作
			if($status == 1002){
				if($data['CheckFlg'] == '0'){
					return 'CheckFlg is not passed';//预校验标识不通过，则结束当前运单号的此函数操作
				}
			}
			
			// 数据表中的状态不是对应1400(即未更新)，若两者相等，即已经执行过更新和保存，无需操作
			if($list['IL_state'] != $status){
				$up_list_sql = "UPDATE mk_tran_list SET IL_state='$status', ex_context='$content', ex_time='$data[CreateTime]' WHERE id = '$list[id]'";

				$add_logs_sql = "INSERT INTO mk_logs (CID,tranid,transit,tranNum,mStr1,MKNO,weight,state) VALUES ('$list[CID]','$list[noid]','$info[transit]','$data[LogisticsNo]','$info[no]','$list[MKNO]','$list[weight]','$status')";

				$add_il_sql = "INSERT INTO mk_il_logs (MKNO,content,create_time,status,noid,state,CID) VALUES ('$list[MKNO]','$content','$data[CreateTime]','$status','$list[noid]','0','$list[CID]')";
				
				if($pdo->exec($up_list_sql) !== false && $pdo->exec($add_logs_sql) !== false && $pdo->exec($add_il_sql) !== false){
					return true;
				}else{
					return false;
				}
	...
	function sendERP($data,$pdo,$erp_url,$erp){
		...
		$context = (isset($data['Result']) && trim($data['Result']) != '') ? htmlspecialchars_decode($data['Result']) : (($status == 1400) ? '发送订单信息到电子口岸' : '清关过程出现问题，正在处理中');
