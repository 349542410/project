 ...
 */
	...
	$find_sql = "SELECT $no,MKNO,id FROM mk_tran_list WHERE IL_state <> '1003' AND trankd = '$TranKd' AND id > $maxlid AND $no <> '' ORDER BY id ASC LIMIT $limit";
		...
		foreach($nu_list as $kk=>$item){
			...
			if(isset($reArr['Head']) && $reArr['Head'] == 'OK'){
				...
				if(count($Route) == 1){
					...
					//170607 Man
					if(MKIL_State($list['opcode'])==1003){
						// 额外保存本公司专有信息
						$new_a[1]['BusinessLinkCode'] = '1003';
						$new_a[1]['TrackingContent']  = "已签收,感谢使用美快国际物流,期待再次为您服务";
						$new_a[1]['OccurDatetime']    = date('Y-m-d H:i:s');
					}
					        ...
					        if(isset($row2['remark'])){
								...
								if($row2['opcode'] == '8000'){
									...
									if(MKIL_State($list['opcode'])==1003){
										// 额外保存本公司专有信息
										$new_a[$key+1]['BusinessLinkCode'] = '1003';
										$new_a[$key+1]['TrackingContent']  = "已签收,感谢使用美快国际物流,期待再次为您服务";
										$new_a[$key+1]['OccurDatetime']    = date('Y-m-d H:i:s');
									}
								}
