<?php
	...
	function save($arr){
		...
		$MKNO = $arr['lastResult']['nu'];	//美快单号
		$data = $arr['lastResult']['data'];	//物流信息

		$sql        = "select CID from mk_tran_list where `MKNO` = '$MKNO' LIMIT 1";
		$first = $pdo->query($sql);
		// $first->execute();
		...
		for($key=$count-1;$key>-1; $key--){
            ...
            $check_mkil[$key] = "SELECT * FROM mk_il_logs WHERE `MKNO` = '$MKNO' AND `content` = '$item[TrackingContent]' AND `create_time` = '$item[OccurDatetime]' AND `status` = '$item[BusinessLinkCode]'";
            ...
            if($count_item[$key] == 0){
                ...
                $sql_mkil[$key] = "INSERT INTO mk_il_logs (MKNO,content,create_time,status,CID) VALUES ('$MKNO', '$item[TrackingContent]', '$item[OccurDatetime]',$item[BusinessLinkCode],$mkcid)";
