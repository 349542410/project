...
class PrintPaybackController extends HproseController{
	...
	public function verify($arr, $type){
    	...
    	//充值的时候数据库记录的人民币金额  单位分
    	$amount = round(floatval($checkOrder['amount']) * 100);

    	//充值的时候数据库记录的人民币金额 与 回调返回的人民币金额
    	if(sprintf("%.2f", $amount) - sprintf("%.2f", $cash_fee) != 0){
    		return array('state'=>'no','msg'=>'支付金额有误');
    	}

    	//充值金额 + 账户余额
    	$last_amount = $checkOrder['amount_usa'] + $checkUser['amount'];
	...
	//回调数据的综合处理方法
	public function console($data, $type){
