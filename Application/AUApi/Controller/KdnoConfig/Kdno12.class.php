 ...
 * 打单/下单
 * 打印MK单
 * 指导文档：IBS平台客户API对接口技术规范-v1.2.pdf
 */
class Kdno12{
/**
 ...
 */
	...
	//基本配置   20160905 Jie 改为由Kdno12.conf.php中传入配置
	...
	//默认的订单信息配置  20160905 Jie 部分配置改为由Kdno12.conf.php中传入配置
	protected $Order_config = array(
		'orderid'         => '',		//客户订单号 (唯一约束)
		// 'is_gen_bill_no'  => '1',		// N 默认值1
		
		// 'j_custid'        => '',			// N 设置在config中
		// 'j_email'         => '',			// N 设置在config中
		// 'j_company'       => '',        	// N 寄件方公司名称，设置在config中
		// 'j_area_code'     => '',			// N 寄件方联系区号，设置在config中
		'j_country'       => 'US',			//寄件方国籍   默认值US，设置在config中
		
		//20170710 V1.20版本新增
		// 'j_province'      => '',			// N 寄件方省份   设置在config中  V1.20
		// 'j_city'          => '',			// N 寄件方城市   设置在config中  V1.20
		// 'j_county'        => '',			// N 寄件人所在县/区，必须是标准的县/区称谓，示例：“福田区”。 设置在config中  V1.20
		
		'j_contact'       => '',        //寄件方联系人，设置在config中
		'j_tel'           => '',		//寄件方联系电话，设置在config中
		// 'j_mobile'        => '',		// N 寄件方手机号码，设置在config中 V1.20
		'j_address'       => '',		//寄件方详细地址，设置在config中
		'j_post_code'     => '',		//寄件方邮编，设置在config中
		
		// 'd_custid'        => '',		// N 到件方月结卡号 V1.20
		...
		// 'reference_no2'   => '',		// N 20160927 将 阿荣传过来的，price 和 discount 生成 price_discount 保存到顺丰reference_no2中
		'express_type'    => '202',		//快件类型	默认值200
		...
		'tax_pay_type'       => '1',	//税金付款方式 1：寄付，2：到付    设置在config中
		'tax_set_accounts'   => '0010002117',	//税金结算账号   设置在config中
	);
...
/*		// Jie 20160923  字段“buyers_nickname” 的容错处理
		// 处理办法：阿荣传输来的有值则用他的，否则用$Order_config中的填补；假如$Order_config中此值为空，则用$orderconf(Kdno12.conf.php里面的数组变量)的值填补；假如$orderconf中的值也是空的，则用最后的默认值"Megao"填补
		...
		foreach($order as $key=>$v){
			$order[$key]['amount'] = sprintf("%.2f",floatval($v['amount'])/$rate);  // Jie 20160927 商品单价/(1+0.119)
		}
		...
		// 发送下单请求
		$OrderResponse = $this->useSoap($data[0], $data[1], $list['orderid'], $this->config['pmsLoginAction']);
...
/*		dump($OrderResponse);
		...
		if($orderArr['Return'] != '' && isset($orderArr['Return'])){
			...
			//发送查询请求
			$OrderSearchResponse = $this->useSoap($sear[0], $sear[1], $list['orderid'], $this->config['pmsLoginAction']);
...
/*		// Jie  20160923  reference_no1 先将-改为_再进行截取，以防存在-
		...
		// 20160927 Jie 此函数中切勿再加载Kdno12.conf.php！！！
...
//========================= 证件图片上传 =======================================
	...
	//由于传统清关需要客户身份证图片信息，商家可以通过此接口上传客户身份证图片信息用于完成清关。
	public function uploadIMG($arr){
		header('Content-type:text/html;charset=UTF-8');	//设置输出格式

		/* 20160905 Jie 载入配置信息 然后 把配置信息写入以下两个变量中 */
		$kdfile 	= dirname(__FILE__).'/Kdno12.conf.php'; 
		require_once($kdfile);//载入配置信息

		$this->config = array_merge($this->config, $baseconf);//用$baseconf数组里面的数组覆盖$Order_config

		// //图片需经过base64加密处理
		// $arr['image'] = base64_encode($arr['image']);
		//$xml = json_encode($arr,JSON_UNESCAPED_UNICODE);
		$xml = json_encode($arr);

		 // echo $xml;die;
		// file_put_contents(ADMIN_ABS_FILE."/Uploads/Person/test.txt", $xml);die;
		// die;
		//对xml报文进行base64加密
...
/*		include_once('HessianPHP/HessianClient.php');
		...
		$http = new HessianClient($this->config['imgUploadAction']); 
        $result = $http->uploadIdentity($data, $validateStr, $customerCode);*/
		
		$Hessian  = new \Libm\HessianPHP\HessianApi();

		$Hessian->url          = $this->config['imgUploadAction'];
		$Hessian->data         = $data;
		$Hessian->validateStr  = $validateStr;
		$Hessian->customerCode = $customerCode;
		// dump($Hessian);
		$result = $Hessian->post();
        ...
        //{"result":"true "}
        //失败响应：
        //{"result":"false","errcode":"50021","failreason":"请求参数不合法"}
		// var_dump($result);
		return json_decode($result,true);//转为数组
	}
...
//======================================================================
	 ...
	 * @return [type]              [description]
	 */
	public function useSoap($data, $validateStr, $orderid, $postUrl){
		set_time_limit(0);

		$soap = new \SoapClient($postUrl);//网络服务请求地址
		...
		if($this->config['exports_switch'] === true){
			...
			$file_name = 'Kdno12_'.$orderid.'_'.time().'.txt';	//文件名
