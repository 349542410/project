...
class AutoPrintSysController extends HproseController{
	 ...
	 */
	public function _info($no, $RMB_Free_Duty, $US_TO_RMB_RATE){

		try{

			$map['random_code'] = array('eq',$no); //凭证号
			$map['order_no']    = array('eq',$no);// Q开头的单号 或者 MQ开头的单号
			$map['_logic']      = 'or';
			$where['_complex']  = $map;
	 ...
	 * @return [type]                   [description]
	 */
	public function _step_one($id, $weight, $time, $operator_id, $RMB_Free_Duty, $US_TO_RMB_RATE, $terminalCode){
		...
		\Think\Log::write('拉斯维加斯打印' . json_encode($terminalState,320));
		if(!$terminalState){
		...
		if((int)$terminalState['point_id'] != 6){
			//验证身份证号，及身份证照片
		...
		try{
			...
			$cost           = number_format(($tax + $cost),2,'.',''); // 总消费金额（包含税金）
			$original_price = number_format(($tax + $original_price),2,'.',''); // 未计算所有折扣优惠的 消费金额（包含税金）
	...
	public function _step_two($id, $operator_id, $terminal_code){
		...
		$cost        = number_format(($freight + $tax + $extra_fee + $real_charge),2,'.','');

        if($user_amount < $cost){
        	return array('state'=>'no','msg'=>'账户余额不足以支付订单，请先充值', 'lng'=>'balance_not_enough_to_pay');
        }

        $user_amount = number_format(($user_amount - $cost),2,'.',''); //新余额 = 原余额-消费金额
	...
	public function _step_three($id, $status, $time, $MKNO, $STNO, $terminal_code, $operator_id){
		...
		try{
			...
			if($check_terminal['state'] == 'no'){
				...
				// 查询条件
				$many_map['member_id']   = array('eq', $info['user_id']);//会员id
				$many_map['order_id']    = array('eq', $info['id']);//订单id
				// $many_map['terminal_id'] = array('eq', $Terminal['id']);//终端号id
				...
				if($check_relation){
					...
					// unset($many_map['terminal_id']);

					// 根据会员id和订单id查询 关系表 的信息
					$error_list = M('PrintRelationOrder')->where($many_map)->find();
					$error_list['receive_terminal_code_id'] = $Terminal['id'];//终端发送过来的 终端号id
					$error_list['receive_terminal_code']    = $terminal_code;//终端发送过来的 终端号

		            $DataNote               = new \Libm\DataNotes\DataNote();
		            $DataNote->RequestData  = $info['order_no'];
		            $DataNote->ResponseData = $error_list;
		            $DataNote->save_dir     = C('AutoSys_Set.Error_Notes');
		            $DataNote->file_name    = C('AutoSys_Set.AutoPrintSysLogs');
		            $file_name = $DataNote->save();

/*	                //发送错误日志邮件
	                $fileurl = C('AutoSys_Set.Error_Notes').$file_name;

	                $args = array(
	                    'to' => array(
	                        '2565855778@qq.com',     //收件人地址，可填写多个
	                    ),
	                    'FromName'   => '美快打印系统',    //发件人姓名，可选
	                    'title'      => '打印第三步的错误',
	                    'content'    => '打印第三步返回：无法找到对应的数据进行更新',
	                    'type'       => 'html',
	                    'attachment' => array(
	                        $fileurl,      //附件的路径，可选
	                    ),
			...
			if($save !== false){
				...
				\Think\Log::write('收寄--写入外部订单号' . json_encode(M('TranUlist')->getLastSql(),320));
				if(!empty($packageId['package_id'])){
					M('TranList')->where(['MKNO' => $MKNO])->save(['package_id' => $packageId['package_id']]);
					\Think\Log::write('收寄--写入外部订单号' . json_encode(M('TranList')->getLastSql(),320));
				}
