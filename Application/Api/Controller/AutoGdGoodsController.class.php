...
/**
	...
	* 修改日期：2017-03-26
	* 用途：自动 商品报备  广东邮政
	* 数据表：mk_apply_list  mk_tran_list_notes
 */
namespace Api\Controller;
use Think\Controller;
class AutoGdGoodsController extends Controller{
	...
	public function index(){
		...
		require_once('GoodsCustoms.class.php');//加载类

		$EMS = new \Custom();

		echo "\r\n".date('Y-m-d H:i:s')."\r\n";
		
		$node = (I('node')) ? trim(I('node')) : 'Gd_ems_goods';//默认 Hk_ems 香港E特快
		...
		$map['apply_status'] = array('eq',0);
		$map['id']           = array('gt',$maxlid);
		$nu_list = M('ApplyList')->where($map)->order('id asc')->limit($this->limit)->select();
