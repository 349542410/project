...
class CategoryController extends HproseController{
...
/*        $where = array();
    ...
    //顶级类别添加 方法
    public function _add_top_method($data){

        //首先对选中的线路进行单个线路排查 顶级类别 是否已经存在  20170922
        $lines = explode(',',trim($data['TranKd'], ','));

        $where['fid']      = array('eq', $data['fid']);
        $where['cat_name'] = array('eq', $data['cat_name']);
        foreach($lines as $li){
            //20170922
            $where['TranKd']   = array('like', "%,".$li.",%");
            $check_name = M('CategoryList')->where($where)->find();
            //验证是否已经存在这个名字
            if($check_name){
                $line_name = M('TransitCenter')->where(array('id'=>$li))->getField('name');
                $result = array('state'=>'no', 'msg'=>'【'.$line_name.'】线路已经有此类别名称');
                return $result;
            }
        }
        ...
        $map['TranKd']   = array('eq', $data['TranKd']);//线路id组合

        $first = M('CategoryList')->where($map)->find();
        //检查这个线路ID组合和顶级类别名称是否已经存在
        if($first){
    ...
    public function _edit($id){
        ...
        return array($center, $info);
    }

    //顶级类别编辑 方法
    public function _edit_top_method($id, $data, $lineId){

        $where1['fid']      = array('eq',$data['fid']);
        $where1['cat_name'] = array('eq',$data['cat_name']);
        $where1['TranKd']   = array('eq',$data['TranKd']);
        $where1['id']       = array('neq',$id);

        //检查这个组合存在的数量
        $first = M('CategoryList')->where($where1)->count();
        ...
        if($first < 1){
            ...
            foreach($lines as $li){
                ...
                //验证是否已经存在这个名字
                if($check_name){
                    if($check_name['TranKd'] != $data['TranKd']){
                        $line_name = M('TransitCenter')->where(array('id'=>$li))->getField('name');
                        $result = array('state'=>'no', 'msg'=>'【'.$line_name.'】线路已经有此类别名称');
                        return $result;
                    }
        ...
        // $newdata['is_show'] = $data['is_show']; // 不跟随父级类别的变动
        // $newdata['status']  = $data['status'];// 不跟随父级类别的变动

        $res = M('CategoryList')->where($map)->save($newdata);
