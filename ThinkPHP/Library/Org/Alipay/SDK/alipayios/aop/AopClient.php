...
class AopClient { 
	...
	function __construct($alipay_config){
		...
		$this->rsaPrivateKey 		= $alipay_config['private_key'];
		$this->alipayPublicKey 		= $alipay_config['alipay_public_key'];
	}
	...
	protected function sign($data, $signType = "RSA2") {	
		...
		if($this->checkEmpty($this->rsaPrivateKeyFilePath)){ 	// checkEmpty检验是否为空 rsaPrivateKeyFilePath文件
			$priKey=$this->rsaPrivateKey; 		// 密匙
		...
		($res) or die('adcd您使用的私钥格式错误，请检查RSA私钥配置'); 
