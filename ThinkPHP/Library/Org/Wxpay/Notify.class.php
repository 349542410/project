...
class Notify{    
    ...
    */
    public function verifyNotify($kind = ""){
        ...
        $file = dirname(__FILE__).'/logs/'.time().'.txt';
        try{
            $file = dirname(__FILE__).'/logs/'.time().'.txt';
            //file_put_contents($file,json_encode($returnValues));
        } catch (Exception $e) {
        }
        ...
        if(count($returnValues["data"]) > 0 ){
            ...
            $data = array(
                ...
                //应返回订单金额，因这个金额需与支付生成的金额相同，所以只写返回，不改字段名
                //"cash_fee"          => $arr['cash_fee'], //支付金额 单位分
                "cash_fee"          => $arr['total_fee'],//$arr['cash_fee'], 支付金额 单位分
