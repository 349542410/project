...
class WechatAuth {
    ...
    /* 二维码类型常量 */
    const QR_SCENE       = 'QR_SCENE';          // 临时
    const QR_LIMIT_SCENE = 'QR_LIMIT_SCENE';    // 永久
     ...
     */
    public function __construct($owner='mg', $token = null){
        //man 160224
        include('conifg.php');
        $appid  = isset($config[$owner]['appid'])?$config[$owner]['appid']:'';
        $secret = isset($config[$owner]['secret'])?$config[$owner]['secret']:'';
        if($appid && $secret){
            $this->appId     = $appid;
    ...
    public function getAccessToken($type = 'client', $code = null){
        ...
        //Man160225增加保存access_token
        $atfile     = C('ATFILE').$type.$param['appid'].'.json';
        if($type=='client'){
            @$data = json_decode(file_get_contents($atfile));
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                ...
                //$atd['expire_time']   = time() + 7000;
                $atd['expire_time']   = time() + 60;
                $atd['access_token']  = $this->accessToken;
                
                //$fp = fopen($atfile, "w");
                //fwrite($fp, json_encode($atd));
                //fclose($fp);
                //可使用以下方式，
                file_put_contents($atfile,json_encode($atd));
                //=====

                if($code==null){
                    return $this->accessToken;
                }else{ //存在code时为 读取登录者open_id
                    return $token; // Man 160302
                }
     ...
     */
    public function getUserInfo($openid,$upid,$lang = 'zh_CN'){
        //if($this->accessToken==''){
            //self::getAccessToken();
        //}
        $query = array(
            'access_token' => self::getAccessToken(),//$this->accessToken, //Man 160302
            'openid'       => $openid,
            'lang'         => $lang,
        );
        $info = self::http("{$this->oauthApiURL}/userinfo", $query);
        $infoj  = json_decode($info, true);
        return $infoj;
    }
    ...
    public function mediaUpload($filename, $type){
        ...
        return $this->api('media/upload', $data, 'POST', $param, false);
    }

    public function uploadMedia($filename,$url){
        $file = quoted_printable_encode($filename); //要上传的文件
        $fields['media'] = '@'.$file;
        $fields['type'] = 'image';
        $ch = curl_init() ;
        curl_setopt($ch, CURLOPT_URL, "$url");
        //curl_setopt($ch, CURLOPT_SSLCERT,'D:/sys/web/thinkphp_3.2.3_full/crm/Advert/Controller/cacert.pem');
    ...
    //Man 160316
    private function ChangeMember($wxid, $grp=1){
        $grpid  = C('GROUPID'.$grp.'.id');
        $grpnm  = C('GROUPID'.$grp.'.name');
        $rs     = self::groupsMemberUpdate($wxid,$grpid);
        //var_dump($rs);
        if($rs['errcode']==0){
            self::sendText($wxid, '恭喜您成为'.$grpnm);
            return true;
        }else{
            if($rs['errcode']==40003){
                //wxid 错的
            }else{
                self::sendText($wxid, '你的级别没有被正确设置，请与上级联系');
            }
            return false;
        }
    }
    public function UpMember($wxid){
        return self::ChangeMember($wxid,2);
    }
    public function DownMember($wxid){
        return self::ChangeMember($wxid,1);
    }
     ...
     */
    public function userInfo($openid,$upid=1,$lang = 'zh_CN'){
        $param  = array('openid' => $openid, 'lang' => $lang);
        $info   = $this->api('user/info', '', 'GET', $param);        
        if(!is_array($info) || $info['subscribe']==0){
            return array('errcode'=>10,'errmsg'=>'未找到你的微信资料');
        }
        $sex    = array('女','男');
        $s      = $sex[$infoj['sex']];
        return array(
            ...
            "wx_avatar"     => $info['headimgurl'],
            "wx_sex"        => ($s=='')?'密':$s,
            "wx_district"   => $info['country'].' '.$info['province'].' '.$info['city'],
            "l1_user_id"    => $upid,
        );
    }
    ...
    * Man 160304 创建个性化菜单
    * data 菜内容，data
      operator 操作类型 ：create,delete
    */
    public function personalMenu(){
        require('menu.php');
        if(!isset($button) || !is_array($button)) return false;
        $bi     = count($button);
        if($bi<2) return '请在wechatauth中取消此项，此项本来用于美快国际';
        foreach ($button as $value) {
            if(!is_array($value)){
                return 'button Error';
            }
        }
        $r[]    = $this->menuDelete();                         // 先删除所有菜单
        foreach ($button as $key=>$value) {
            if($key==0){
                $r[]    = $this->menuCreate($value);             // 创建默认菜单
            }else{
                $r[]= $this->api('menu/addconditional',$value);
            }
        }
        return $r;
    }
    ...
    /*
    * Man 160304 下载并保存分销商的推广二维码,未完成
    * scene_str 会员id
    * path 保存的绝对路径 
    * force =true 强制生成二维码图片，即使文件已经存在
    */
    public function qrcodeDownsave($scene_str,$path,$force=false){
        if($scene_str=='' || $path==''){
            return false;
        }
        $fn     = $path.md5($scene_str).'.png';
        if(file_exists($fn) && !$force) return true;

        $scene  = self::qrcodeCreate($scene_str);
        //var_dump($scene);
        //{"ticket":"","expire_seconds":60,"url":"http:\/\/weixin.qq.com\/q\/kZgfwMTm72WWPkovabbI"}
        if(!$url=$scene['url']) return false;
        $url    = urldecode($url);
        //echo $url;
        Vendor('phpqrcode.phpqrcode');
        \QRcode::png($url,$fn,QR_ECLEVEL_H,4,2,true);
     ...
     * @param $scene_str Man160302
     */
    public function qrcodeCreate($scene_str='1', $scene_id=null,$expire_seconds = 0){
        $data = array();
        ...
        if(is_numeric($expire_seconds) && $expire_seconds > 0){
            ...
            $data['action_name']    = (isset($scene_id))?self::QR_LIMIT_SCENE:'QR_LIMIT_STR_SCENE';
        }
        if(isset($scene_id)){
    ...
    protected function api($name, $data = '', $method = 'POST', $param = '', $json = true){
        //$params = array('access_token' => $this->accessToken); //Man160225
        $params = array('access_token' => self::getAccessToken());
        if(!empty($param) && is_array($param)){
            $params = array_merge($params, $param);
        }

        $url  = "{$this->apiURL}/{$name}";
        //$url  = 'http://file.api.weixin.qq.com/cgi-bin/'.$name;
        if($json && !empty($data)){
            //保护中文，微信api不支持中文转义的json结构
    ...
    protected static function http($url, $param, $data = '', $method = 'GET'){
        ...
        if(strtoupper($method) == 'POST'){
            ...
            if(is_string($data)){ //发送JSON数据,Man160303如果data为数组时则不进行以下处理
