...
class Wechat {
    ...
    public function __construct($owner = 'mg'){
        ...
        $key    = isset($config[$owner]['key'])?$config[$owner]['key']:'';
        //参数验证
        if(self::$msgSafeMode){
        ...
        //TOKEN验证
        $token  = isset($config[$owner]['token'])?$config[$owner]['token']:false;
        if($token){
            self::auth($token) || exit;

            if(IS_GET){
                exit(htmlentities($_GET['echostr']));
            }else{
                self::$token = $token;
                $this->init();
            }
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
        ...
        //file_put_contents('/home/www/m/Code/Advert/Controller/ttt.json',json_encode($content));
        ...
        $xml = self::data2xml(null, $data);
        //file_put_contents('/home/www/m/Code/Advert/Controller/ccc.json',$xml->asXML());
        exit($xml->asXML());
    }
    ...
    public function replyText($text){
        //return $this->response(array('Content'=>$text), self::MSG_TYPE_TEXT);
     ...
     */
    public function replyService($KfAccount=null){
        /* 基础数据 */
        $data = array(
            'ToUserName'   => $this->data['FromUserName'],
            'FromUserName' => $this->data['ToUserName'],
        ...
        //如何指定客服
        if($KfAccount){
            $data['TransInfo']['KfAccount'] = $KfAccount;
        }
     ...
     * @return string
     */
    protected static function data2xml_2($xml, $data, $item = 'item') {
    ...
    /*Man 160320 多客服
    */
    private static function transfer_customer_service($KfAccount=null){
        if($KfAccount==null) return null;
    }
